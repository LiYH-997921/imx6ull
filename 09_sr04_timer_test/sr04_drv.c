...
static struct gpio_desc gpios[2] = {
    {115, 0, "trig", },//gpio4_19  //(4-1)*32+19
    {116, 0, "echo", },//gpio4_20  //(4-1)*32+20
};
...
static ssize_t sr04_read (struct file *file, char __user *buf, size_t size, loff_t *offset)
{
	...
	if (is_key_buf_empty() && (file->f_flags & O_NONBLOCK))
		return -EAGAIN;
	//printk("%s %s line %d\n", __FILE__, __FUNCTION__, __LINE__);//打印函数内部关了中断
...
static long sr04_ioctl(struct file *filp, unsigned int command, unsigned long arg)
{
	switch(command){
		case CMD_TRIG:
		{
			...
			//启动定时器，并修改定时器超时时间
			mod_timer(&gpios[1].key_timer, jiffies+msecs_to_jiffies(50));//msecs_to_jiffies(50):相当于HZ/200
		}
...
static irqreturn_t sr04_isr(int irq, void *dev_id)
{
	...
	if(val){//上升沿
		...
		put_key(time);//将数据转成us
		wake_up_interruptible(&gpio_wait);//唤醒
...
{
		put_key(-1);//返回-1，让应用程序知道是定时器超时，没读到数据
		wake_up_interruptible(&gpio_wait);//唤醒
		kill_fasync(&button_fasync, SIGIO, POLL_IN);//发送信号
...
static int __init sr04_init(void)
{
	...
	gpio_direction_output(gpios[0].gpio, 0);//设置trig引脚输出低电平
		
	//设置echo引脚中断
	gpios[1].irq  = gpio_to_irq(gpios[1].gpio);
	err = request_irq(gpios[1].irq, sr04_isr, IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING, "100ask_sr04", &gpios[1]);
...
static void __exit sr04_exit(void)
{
	...
	//删除定时器，相当于关闭定时器
	del_timer(&gpios[1].key_timer);//如果不删除定时器会导致驱动无法卸载
